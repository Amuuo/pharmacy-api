...
public interface IPharmacyService
{
    ...
    /// <returns>
    /// A Response containing IAsyncEnumerable of Pharmacy objects if any match the search criteria,
    /// or an error message if no matching pharmacies are found or if an exception occurs during retrieval.
    /// </returns>
    Task<IResult<IPagedResult<Pharmacy>?>> GetPharmacyListPagedAsync(PagingInfo pagingInfo);
    ...
    /// <returns>
    /// A Response containing the updated Pharmacy object if successful,
    /// or an error message if no pharmacy is found with the given id or if an exception occurs during the update.
    /// </returns>
    Task<IResult<Pharmacy?>> UpdatePharmacyAsync(Pharmacy updatedPharmacy);
    ...
    /// <returns>
    /// A Response containing the Pharmacy object if found,
    /// or an error message if no pharmacy record with the specified id exists.
    /// </returns>
    Task<IResult<Pharmacy?>> GetPharmacyByIdAsync(int id);
    ...
    /// <returns>
    /// A Response containing the inserted Pharmacy object if successful,
    /// or an error message if an exception occurs during the insertion.
    /// </returns>
    Task<IResult<Pharmacy?>> InsertPharmacyAsync(Pharmacy? newPharmacy);
    ...
    /// <returns>
    /// A Response containing IAsyncEnumerable of Pharmacy objects associated with the specified pharmacist,
    /// or an error message if no pharmacies are found or if an exception occurs during retrieval.
    /// </returns>
    Task<IResult<IEnumerable<Pharmacy>?>> GetPharmaciesByPharmacistIdAsync(int pharmacistId);
}