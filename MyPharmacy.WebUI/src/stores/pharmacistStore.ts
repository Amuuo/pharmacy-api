...
export const fetchPharmacistListFx = createEffect<
  GridPaginationModel,
  any,
  Error
>();
export const fetchPharmacistListByPharmacyIdFx = createEffect<
  number,
  any,
  Error
>();
...
addPharmacistFx.use(async (pharmacist: Pharmacist) => {
  const response = await fetch(
    `${import.meta.env.VITE_API_URL}/pharmacist/add`,
    {
      method: "POST",
...
pharmacistStore
  ...
  .on(addPharmacistFx, (state) => ({ ...state, addingPharmacist: true }))
