...
export default function PharmacistList({
  ...
  const { pharmacistList, loadingPharmacistList, totalCount } =
    useStore(pharmacistStore);
  ...
  const handlePharmacistSelectionChange = (
    newSelectedPharmacist: GridRowSelectionModel,
  ) => {
  ...
  const outgoingStyle = enableAnimation
    ? `${styles.pharmacistGrid} ${styles.outgoing}`
    : `${styles.pharmacistGrid}`;
  const incomingStyle = enableAnimation
    ? `${styles.pharmacistGrid} ${styles.incoming}`
    : `${styles.pharmacistGrid}`;
  ...
  if (loadingPharmacistList) {
    return (
      <LinearProgress
        ...
        sx={{ gridArea: "pharmacist" }}
      />
    );
  } else if (pharmacistList.length === 0) {
    return (
      <h3 style={{ textAlign: "center", gridArea: "pharmacist" }}>
